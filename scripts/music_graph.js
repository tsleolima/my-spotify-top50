...
d3.json('https://raw.githubusercontent.com/tsleolima/my-spotify-top50/master/top50leo.json', function (error, graph) {
    ...
    // result.edges = [];
    // edges = [];
    // Filtro para criar nós apartir da api do spotify

    // for (var index = 0; index < graph.items.length; index++) {
    //     var artista = graph.items[index];
    //     var node = {};
    //     node.id = artista.id;
    //     node.name = artista.name;
    //     node.genres = artista.genres;
    //     node.img = artista.images[0].url;
    //     node.url = artista.external_urls.spotify;
    //     result.nodes.push(node);
    // }

    // Filtro para criar arestas apartir da api do spotify

    // for (var i = 0; i < graph.nodes.length; i++) {
    //     var generosPrimario = graph.nodes[i].genres;
    //     for (var j = 1; j < graph.nodes.length; j++) {
    //         var generosSecundario = graph.nodes[j].genres;
    //         for (var k = 0; k < generosPrimario.length; k++) {
    //             for (var l = 0; l < generosSecundario.length; l++) {
    //                 if (generosPrimario[k] == generosSecundario[l]) {
    //                     var aresta = {};
    //                     aresta.source = graph.nodes[i].id;
    //                     aresta.target = graph.nodes[j].id;
    //                     aresta.type = graph.nodes[i].genres[k];
    //                     edges.push(aresta);
    //                 }
    //             }
    //         }
